...
<body>
    <div> 
        <div id="categorytable" style="margin:10px">
             <table id="categorylist" style="display:none"></table>
        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var option = {
                height: gh / 2 - 5, //flexigrid插件的高度，单位为px
                width: w,
            ...
            var caoption = {
                height: gh / 2 - 5, //flexigrid插件的高度，单位为px
                width: w,
                url: '<%=Url.Action("GetCategoryListData")%>', //ajax url,ajax方式对应的url地址 
                colModel: [
			        { display: '分类ID', name: 'CategoryID', width: 60, sortable: false, align: 'left' },
			        { display: '分类名称', name: 'CategoryName', width: 100, sortable: false, align: 'left' }
				],
                title: "分类列表",
                usepager: false,
                rowbinddata: true,
                rowhandler: reflashproject
            };
            $("#categorylist").flexigrid(caoption);
            $("#productlist").flexigrid(option);
            function formatMoney(value, pid) {
                return "￥" + parseFloat(value).toFixed(2);
            }
            function reflashproject(row) {
                $(row).click(function() {
                    var id = this.id.substr(3);
                    var p = { extParam: [{ name: "categoryId", value: id}] };
                    $("#productlist").flexOptions(p).flexReload();
                });
            }
